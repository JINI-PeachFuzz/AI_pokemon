...
public class FileDownloadService {
    ...
    public void process(Long seq) {
        ...
        fileName = new String(fileName.getBytes(), StandardCharsets.ISO_8859_1); // 8859_1 이게 유니코드 2바이트 형태로 바꿔줬음/ 윈도우는 3~4바이트임
        ...
        try (FileInputStream fis = new FileInputStream(file);
            ...
            response.setHeader("Cache-Control", "no-cache"); // 다운받는데 이미 있으면 안되니까
            response.setHeader("Pragma", "no-cache"); // 옛날 브라우저용 Pragma
            response.setIntHeader("Expires", 0); // 만료시간을 없앤다. / 용량이 너무많으면 영영 못받음
